...
Слой work_format описывает ТОЛЬКО общий рабочий формат:
- как человек включается в работу;
- какой темп и формат задач ему подходят;
- где может терять включённость;
- как руководителю использовать этот формат;
- что проверить через интервью/кейс.
...
Описывай общий рабочий формат, не hiring verdict.
...
function collectWorkFormatBaseTextFields(layer: Record<string, unknown>): BaseTextField[] {
...
export function scanWorkFormatBaseFitHireLanguage(
  layer: Record<string, unknown>,
): OffendingMatch[] {
  return scanPatternInBaseFields(
    collectWorkFormatBaseTextFields(layer),
    FIT_HIRE_FORBIDDEN_PATTERNS,
  );
}

export function scanWorkFormatBaseForbiddenHdTerms(
  layer: Record<string, unknown>,
): OffendingMatch[] {
  const matches: OffendingMatch[] = [];

  for (const field of collectWorkFormatBaseTextFields(layer)) {
...
export function findWorkFormatBaseBannedTerms(layer: Record<string, unknown>): string[] {
  ...
  for (const match of scanWorkFormatBaseForbiddenHdTerms(layer)) {
...
export function validateWorkFormatLayer(layer: Record<string, unknown>): LayerValidationResult {
  ...
  const baseFields = collectWorkFormatBaseTextFields(layer);
  ...
  const fitHireMatches = scanWorkFormatBaseFitHireLanguage(layer);
  ...
  const hdTermMatches = scanWorkFormatBaseForbiddenHdTerms(layer);
