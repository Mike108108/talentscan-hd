...
export default function App() {
  ...
  // City autocomplete for HD audit
  const [hdAuditCityInput, setHdAuditCityInput] = useState("");
  const [hdAuditCitySelected, setHdAuditCitySelected] = useState<GeocodeSuggestion | null>(null);
  const [hdAuditCitySuggestions, setHdAuditCitySuggestions] = useState<GeocodeSuggestion[]>([]);
  const [hdAuditCitySearching, setHdAuditCitySearching] = useState(false);
  const [hdAuditCityDropdownOpen, setHdAuditCityDropdownOpen] = useState(false);
  const hdAuditCityDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  function handleHdAuditCityInput(val: string) {
    setHdAuditCityInput(val);
    setHdAuditCitySelected(null);
    setHdAuditResult(null);

    if (hdAuditCityDebounceRef.current) clearTimeout(hdAuditCityDebounceRef.current);

    if (val.trim().length < 2) {
      setHdAuditCitySuggestions([]);
      setHdAuditCityDropdownOpen(false);
      return;
    }

    hdAuditCityDebounceRef.current = setTimeout(async () => {
      setHdAuditCitySearching(true);
      try {
        const token = await getAccessToken();
        if (!token) return;
        const response = await fetch(
          `/.netlify/functions/geocode-city?q=${encodeURIComponent(val.trim())}`,
        ...
        setHdAuditCitySuggestions(list);
        setHdAuditCityDropdownOpen(list.length > 0);
      } catch {
        setHdAuditCitySuggestions([]);
      } finally {
        setHdAuditCitySearching(false);
      }
    }, 350);
  }

  function handleHdAuditCitySelect(s: GeocodeSuggestion) {
    setHdAuditCitySelected(s);
    setHdAuditCityInput(s.label);
    setHdAuditCityDropdownOpen(false);
    setHdAuditCitySuggestions([]);
    setHdAuditError("");
  }

  function handleHdAuditCityBlur() {
    // Delay to allow click on suggestion to fire first
    setTimeout(() => setHdAuditCityDropdownOpen(false), 180);
  }
  ...
  async function runHdAudit() {
    ...
    if (!hdAuditCitySelected) {
      setHdAuditError(
        "Выберите город из списка — это нужно для точных координат расчёта.",
      );
    ...
    if (!auditDate || !auditTime) {
      setHdAuditError(
        "Заполните дату и время рождения в профиле или в форме «Новый разбор».",
      );
      setHdAuditLoading(false);
      return;
    }
    ...
    try {
      const response = await fetch("/.netlify/functions/hd-chart-debug", {
        ...
        body: JSON.stringify({
          birthDate: auditDate,
          birthTime: auditTime,
          birthCity: hdAuditCitySelected.city || hdAuditCitySelected.label,
          birthLatitude: hdAuditCitySelected.lat,
          birthLongitude: hdAuditCitySelected.lng,
        }),
      });

      const data = (await response.json().catch(() => null)) as
        | (HdAuditResult & { error?: string; hint?: string })
        | null;

      if (!response.ok) {
        const hint = data?.hint ? ` (${data.hint})` : "";
        setHdAuditError((data?.error ?? `Ошибка ${response.status}.`) + hint);
      } else if (data) {
              ...
              <>
                ...
                <div className="hd-audit-card">
                  ...
                  <p className="hd-audit-desc">
                    Технический режим: показывает полный JSON Human Design API,
                    нормализованную карту и координаты.
                    Данные <strong>не сохраняются</strong>.
                  </p>

                  {/* City autocomplete */}
                  <div className="hd-audit-field-group">
                    <label className="hd-audit-field-label">Город рождения</label>
                    <div className="hd-audit-autocomplete-wrap">
                      <div className="hd-audit-autocomplete-input-row">
                        <input
                          className="hd-audit-autocomplete-input"
                          type="text"
                          placeholder="Начните вводить город…"
                          value={hdAuditCityInput}
                          onChange={(e) => handleHdAuditCityInput(e.target.value)}
                          onBlur={handleHdAuditCityBlur}
                          autoComplete="off"
                          spellCheck={false}
                        />
                        {hdAuditCitySearching && (
                          <span className="hd-audit-autocomplete-spinner" aria-hidden="true">…</span>
                        )}
                      </div>

                      {hdAuditCityDropdownOpen && hdAuditCitySuggestions.length > 0 && (
                        <ul className="hd-audit-suggestions" role="listbox">
                          {hdAuditCitySuggestions.map((s) => (
                            <li
                              key={s.id}
                              className="hd-audit-suggestion-item"
                              role="option"
                              onMouseDown={() => handleHdAuditCitySelect(s)}
                            >
                              <span className="hd-audit-suggestion-label">{s.label}</span>
                              <span className="hd-audit-suggestion-coords">
                      ...
                      {hdAuditCitySelected ? (
                        <p className="hd-audit-city-confirmed">
                          ✓ {hdAuditCitySelected.label} — lat {hdAuditCitySelected.lat}, lng {hdAuditCitySelected.lng}
                        </p>
                      ) : hdAuditCityInput.trim().length >= 2 && !hdAuditCitySearching ? (
                        <p className="hd-audit-city-warn">
                          Выберите город из списка, чтобы расчёт был точным
                        </p>
                      ) : null}
                    </div>
                  </div>
                  ...
                  <p className="hd-audit-hint">
                    Дата и время берутся из формы «Новый разбор»; если пустые — из профиля.{" "}
                    {(birthDate || userProfile.birthDate) ? (
                      ...
                      <span className="hd-audit-hint-warn">
                        Дата и время рождения не заполнены
                      </span>
                    )}
                  </p>
                  ...
                  {hdAuditResult && (
                    <div className="hd-audit-result">
                      ...
                      <div className="hd-audit-section">
                        <span className="hd-audit-section-title">Координаты</span>
                        <div className="hd-audit-kv-row">
                          <span className="hd-audit-kv-key">city</span>
                          <span className="hd-audit-kv-val">{hdAuditResult.coordinateStatus.requestedCity}</span>
                        </div>
                        <div className="hd-audit-kv-row">
                          <span className="hd-audit-kv-key">lat / lng</span>
                          <span className="hd-audit-kv-val">
                            {hdAuditResult.coordinateStatus.lat} / {hdAuditResult.coordinateStatus.lng}
                          </span>
                        </div>
                        <div className="hd-audit-kv-row">
                          <span className="hd-audit-kv-key">source</span>
                          <span className="hd-audit-kv-val">{hdAuditResult.coordinateStatus.coordinateSource}</span>
                        </div>
                        <div className="hd-audit-kv-row">
                          <span className="hd-audit-kv-key">isFallback</span>
                          <span className={`hd-audit-kv-val ${hdAuditResult.coordinateStatus.isFallback ? "hd-audit-kv-warn" : "hd-audit-kv-ok"}`}>
                            {hdAuditResult.coordinateStatus.isFallback ? "true ⚠️" : "false ✓"}
                          </span>
                        </div>
                        {hdAuditResult.coordinateStatus.warning && (
                          <p className="hd-audit-city-warn" style={{ marginTop: "0.25rem" }}>
                            {hdAuditResult.coordinateStatus.warning}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="hd-audit-section">
                        ...
                        <p className="hd-audit-keys">{hdAuditResult.hdTopLevelKeys.join(", ") || "—"}</p>
                      </div>

                      {hdAuditResult.hdDataKeys.length > 0 && (
                        <div className="hd-audit-section">
                          <span className="hd-audit-section-title">Ключи внутри data / chart / bodygraph</span>
                          <p className="hd-audit-keys">{hdAuditResult.hdDataKeys.join(", ")}</p>
                        </div>
                      )}
                      ...
                      <div className="hd-audit-section">
                        ...
                        <div className="hd-audit-flags">
                          {(Object.entries(hdAuditResult.detected) as [string, boolean][]).map(
                            ([key, val]) => (
                              <span
                                key={key}
                      ...
                      {/* normalizedChart summary */}
                      <div className="hd-audit-section">
                        <span className="hd-audit-section-title">Нормализованная карта</span>
                        <div className="hd-audit-nchart">
                          <div className="hd-audit-nchart-row">
                            <span className="hd-audit-nchart-label">canRenderBodygraph</span>
                            <span className={`hd-audit-nchart-val ${hdAuditResult.normalizedChart.canRenderBodygraph ? "hd-audit-kv-ok" : "hd-audit-kv-warn"}`}>
                              {hdAuditResult.normalizedChart.canRenderBodygraph ? "true ✓" : `false — не хватает: ${hdAuditResult.normalizedChart.missingForBodygraph.join(", ")}`}
                            </span>
                          </div>
                          <div className="hd-audit-nchart-row">
                            <span className="hd-audit-nchart-label">type</span>
                            <span className="hd-audit-nchart-val">{hdAuditResult.normalizedChart.type}</span>
                          </div>
                          <div className="hd-audit-nchart-row">
                            <span className="hd-audit-nchart-label">profile</span>
                            <span className="hd-audit-nchart-val">{hdAuditResult.normalizedChart.profile}</span>
                          </div>
                          <div className="hd-audit-nchart-row">
                            <span className="hd-audit-nchart-label">strategy</span>
                            <span className="hd-audit-nchart-val">{hdAuditResult.normalizedChart.strategy}</span>
                          </div>
                          <div className="hd-audit-nchart-row">
                            <span className="hd-audit-nchart-label">authority</span>
                            <span className="hd-audit-nchart-val">{hdAuditResult.normalizedChart.authority}</span>
                          </div>
                          <div className="hd-audit-nchart-row">
                            <span className="hd-audit-nchart-label">definedCenters</span>
                            <span className="hd-audit-nchart-val">
                              {hdAuditResult.normalizedChart.definedCenters.join(", ") || "—"}
                            </span>
                          </div>
                          <div className="hd-audit-nchart-row">
                            <span className="hd-audit-nchart-label">openCenters</span>
                            <span className="hd-audit-nchart-val">
                              {hdAuditResult.normalizedChart.openCenters.join(", ") || "—"}
                            </span>
                          </div>
                          <div className="hd-audit-nchart-row">
                            <span className="hd-audit-nchart-label">channels</span>
                            <span className="hd-audit-nchart-val">
                              {hdAuditResult.normalizedChart.channelsShort.join(", ") || "—"}
                            </span>
                          </div>
                          <div className="hd-audit-nchart-row">
                            <span className="hd-audit-nchart-label">gatesPersonality ({hdAuditResult.normalizedChart.gatesPersonality.length})</span>
                            <span className="hd-audit-nchart-val">
                              {hdAuditResult.normalizedChart.gatesPersonality.join(", ") || "—"}
                            </span>
                          </div>
                          <div className="hd-audit-nchart-row">
                            <span className="hd-audit-nchart-label">gatesDesign ({hdAuditResult.normalizedChart.gatesDesign.length})</span>
                            <span className="hd-audit-nchart-val">
                              {hdAuditResult.normalizedChart.gatesDesign.join(", ") || "—"}
                            </span>
                          </div>
                          <div className="hd-audit-nchart-row">
                            <span className="hd-audit-nchart-label">gatesBoth ({hdAuditResult.normalizedChart.gatesBoth.length})</span>
                            <span className="hd-audit-nchart-val">
                              {hdAuditResult.normalizedChart.gatesBoth.join(", ") || "—"}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      {/* Full JSON */}
                      <div className="hd-audit-section">
                        <span className="hd-audit-section-title">hdRaw (полный JSON)</span>
                      ...
                      <div className="hd-audit-section">
                        <span className="hd-audit-section-title">normalizedChart (JSON)</span>
                        <pre className="hd-audit-json">
                          {JSON.stringify(hdAuditResult.normalizedChart, null, 2)}
                        </pre>
                      </div>
                    </div>
                  )}
